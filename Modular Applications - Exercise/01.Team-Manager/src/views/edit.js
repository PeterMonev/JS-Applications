...
let editFormTemplate = (submitEdit, err, teamInfo) => html` <section id="edit">
  <article class="narrow">
    ...
    <form @submit=${submitEdit} id="edit-form" class="main-form pad-large">
      ...
      <label
        >Team name: <input .value=${teamInfo.name} type="text" name="name"
      /></label>
      <label
        >Logo URL: <input .value=${teamInfo.logoUrl} type="text" name="logoUrl"
      /></label>
      <label
        >Description:
        <textarea name="description">${teamInfo.description}</textarea>
      </label>
...
export function showEdit(ctx) {
  ...
  async function submitEdit(ev) {
    ...
    try {
      ...
      let serverResponseData = await editDataToServer(
        data,
        endPoints.browse,
        ctx.page.teamInfo._id,
        endPoints.token()
      );

...
let detailsTemplate = (
  ...
  <article class="layout">
    ...
    <div class="tm-preview">
      ...
      <span class="details"
        >${until(countTeamMembers(teamInfo), "Loading")} Members</span
      >
      <div>
        <a
          style=${currentUserRole === "owner" ? 'display:""' : "display:none"}
          href="/edit"
          class="action"
          >Edit team</a
        >
        <a
          @click=${joinTeam}
          style=${currentUserRole === "non-applicant"
            ? 'display:""'
            : "display:none"}
          href="#"
          class="action"
        ...
        >
        <a
          style=${currentUserRole === "member" ? 'display:""' : "display:none"}
          href="#"
          class="action invert"
          >Leave team</a
        >
        <span
          style=${currentUserRole === "applicant"
            ? 'display:""'
            : "display:none"}
          >Membership pending.</span
        >
        <a
          style=${currentUserRole === "applicant"
            ? 'display:""'
...
export async function showTeamDetails(ctx) {
  ...
  try {
    let [teamInfo, teamUsersInfo] = await Promise.all([
      getDataFromServer(`data/teams/${teamId}`),
      getDataFromServer(
    ...
    let members = teamUsersInfo.filter(
      (teamUser) => teamUser.status === "member"
    );
    let applicants = teamUsersInfo.filter(
      (teamUser) => teamUser.status === "pending"
    );
    ...
    async function joinTeam() {
      ...
      let requestedMembershipResponse = await sendDataToServer(
        { teamId: teamId },
        endPoints.memberStatus,
        token
      );
    ...
    function determineCurrentUserRole() {
      if (!isLogged) {
        ...
        teamInfo._ownerId === JSON.parse(sessionStorage.getItem("userData"))._id
      ) {
        return "owner";
      } else {
        let currentUserId = JSON.parse(sessionStorage.getItem("userData"))._id;
        let currentUserPresentInTeam = teamUsersInfo.filter(
          (membershipInfo) => membershipInfo.user._id === currentUserId
        );
